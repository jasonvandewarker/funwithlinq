...
namespace LinqExercises.Controllers
{
    public class CustomersController : ApiController
    {
        ...
        {
            var customerCity = from customerNames in _db.Customers
                               where customerNames.City.Contains(city)
                               select customerNames;

            return Ok(customerCity);
        ...
        public IHttpActionResult GetAllFromMexicoSwedenGermany()
        {
            ...
            var specificCountry =   from customer in _db.Customers
                                    where (customer.Country == "Mexico" ||
                                    customer.Country == "Sweden" ||
                                    customer.Country == "Germany")
                                    select customer;

            return Ok(specificCountry);
        ...
        //    var shipWith = from customerNames in _db.Customers
        //                   where customerNames.c.Contains(shipperName)
        //                   select customerNames;

        //    return Ok(shipWith);

        //    //Need to get stuff from more than one table ? !!?
